 ...
 * 基础模块，所以模块均可继承此块
 */
class MysqlManager extends CDbCommand
{
	...
	public function lockTable($table){
		...
		$result = $this->ModelExecute($sql);
		if(!$result){
			Yii::log("DBManager::lockTable():executeQuery() sql:".$sql." failed", "log");
			return  false;
		}
		return true;
	}
	...
	function unlockTables(){
		...
		$result = $this->ModelExecute($sql);
		if(!$result){
			Yii::log("DBManager::unlockTables():executeQuery() sql:".$sql." failed", "log");
			return  false;
		}
		return true;
	}
