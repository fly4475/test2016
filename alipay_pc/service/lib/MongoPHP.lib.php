...
class MongoPHP{
    ...
    public function __construct($config=array('host'=>'localhost','port'=>27017,'db'=>'db_yl_bug_report','cmd'=>'$')){
		try{
			...
			Log::write("MongoPHP::_construct() exception, error:".$e->getMessage(), "log");			
		}
    }
    ...
    public function delete($colName,$query=array(),$delAll=true,$safe=false,$fsync=false){
		...
		try{
			...
			$col = $this->_getCol($colName);
			$ret = $col->remove($query,$options);
		 }catch (Exception $e){
        	Log::write("MongoPHP::delete() exception, error:".$e->getMessage(), "log");
        	return false;
        }
     ...
     * @param int $max
     */
    public function createCollection($colName, $capped = false, $size =0, $max = 0){
    	return $this->_db->createCollection($colName, $capped, $size, $max);
    }
     ...
     */
    public function getIndexInfo($colName){
    	return $this->_getCol($colName)->getIndexInfo();
    }
    /**
     * 删除整个集合
     * 
    ...
    public function update($colName,$newDoc,$query=array(),$option='set',$upAll=true,$upsert=false,$safe=false,$fsync=false){
		try {
        	...
        	Log::write("MongoPHP::update() exception, error:".$e->getMessage(), "log");
        	return false;
        }
        return $ret;
    }
    ...
    public function select($colName,$query=array(),$fields=array(),$sort=array(),$limit=0,$skip=0){
    	try{
        	...
        	Log::write("MongoPHP::select() exception, error:".$e->getMessage(), "log");
        	return false;
        }
        return $result;
    }
    ...
    public function count($colName,$query=array(),$limit=0,$skip=0){
    	if(count($query) <= 0){
    		return $this->_getCol($colName)->count();
    	}

    	return $this->_getCol($colName)->count($query);
     ...
     * @return array
     */
    public function group($colName, $keys, $initial, $reduce){
    	$g = $this->_getCol($colName)->group($keys, $initial, $reduce);
...
function unitTest(){
	...
	$g_arr_recod_mongo_db_conn = array(
			"host" =>"localhost",
			"port" =>27017,
			'db'=>'db_yl_bug_report',
			//'cmd'=>'$'
	);
	$mongo = new MongoPHP($g_arr_recod_mongo_db_conn);
...
// 	$mongo->selectDB('bug');
	...
	$mongo->save('user', array('name'=>'yuan'.$i));
