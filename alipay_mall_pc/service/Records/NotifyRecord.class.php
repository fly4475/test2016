...
class NotifyRecord extends Record
{
	...
	{
		$sql = "SELECT order_status,is_used_coupon,coupon_code FROM user_order WHERE order_id = '%s'";
		$sql = sprintf($sql,$this->out_trade_no);
		return $sql;
	}
	...
	public function getUpdatePayTimeAndStatusSql()
	{
		...
		$sql = "UPDATE user_order SET order_status = %d , pay_time = '%s', pay_type =2 WHERE order_id = '%s'";
		$sql = sprintf($sql,$status,$this->notify_time,$this->out_trade_no);
		return $sql;
	}
	
	public function getUpdateCouponStatusSql($real_trade_status)
	{
		$sql = "UPDATE goods_coupon SET is_used = 1 ,update_time = '%s' WHERE coupon_code = '%s'";
		$sql = sprintf($sql,date('Y-m-d H:i:s'),$real_trade_status['coupon_code']);
		return $sql;
	}
	...
	{
		$status = $this->getIntStatus();
		$sql = "UPDATE user_order SET order_status = '%d' , update_time = '%s' WHERE order_id = '%s'";
		$sql = sprintf($sql,$status,$this->notify_time,$this->out_trade_no);
		return $sql;
	}
